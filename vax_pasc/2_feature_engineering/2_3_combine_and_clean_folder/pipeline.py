...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.050a8c76-0469-4f54-afbd-69c30f355142"),
...
@transform_pandas(
    Output(rid="ri.vector.main.execute.7d292f27-d95e-4333-a45c-5f8059aefdae"),
    C_nodes=Input(rid="ri.foundry.main.dataset.cff588b0-76bc-4f8d-84d7-24dfe29934e2"),
    Cci_index=Input(rid="ri.foundry.main.dataset.8a50ad0e-c074-4fed-8ce0-2d3b99636236"),
    Final_withL=Input(rid="ri.foundry.main.dataset.4d171d5f-1ffb-48d3-813e-cb95e4ccb70f"),
    Preprocessed=Input(rid="ri.foundry.main.dataset.d8070d34-be0f-4109-a394-a6f4e1820f32")
)
def Combined_2(Final_withL, C_nodes, Preprocessed, Cci_index):
    df = Final_withL.join(Preprocessed, on=['person_id'], how = 'left').join(C_nodes, on = ['person_id', 't0'], how='left').join(Cci_index, on='person_id', how='left')
...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.b055970f-c7df-48d5-9500-ea0849721c29"),
    type_to_factor=Input(rid="ri.vector.main.execute.a55fdde9-756a-4c08-a49f-847143edb8e6")
)
...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.2cdf5c83-351d-4a15-9e69-b9b403eda392"),
    FINAL=Input(rid="ri.foundry.main.dataset.b055970f-c7df-48d5-9500-ea0849721c29")
)
import re
def Unadjusted(FINAL):
    pattern = re.compile(r'^[A]\d+$')
    selected_columns = [c for c in FINAL.columns if pattern.match(c) or c=='Y7']
    df = FINAL.select(*selected_columns)
    
    schema = StructType([
        StructField("treatment", IntegerType(), True),
        StructField("PASC_count", IntegerType(), True),
        StructField("treatment_count", IntegerType(), True),
        StructField("time", StringType(), True),
        StructField("PASC_prop", DoubleType(), True)
    ])  
    spark = SparkSession.builder.appName("EmptyDataFrameExample").getOrCreate()
    df_all = spark.createDataFrame([], schema)
    
    # df = df.filter(df['A7']==1)
    for c in selected_columns:
        if c not in ['A7', 'Y7']:
            df_sum = df.groupBy(c).agg(F.sum('Y7').alias('PASC_count'), F.count('*').alias('treatment_count')).withColumnRenamed(c, "treatment").withColumn('time', F.lit(c))
            df_sum = df_sum.withColumn('PASC_prop', F.col('PASC_count')/F.col('treatment_count'))
            df_all = df_all.union(df_sum)
    df_all = df_all.select(*['time', 'treatment', 'treatment_count', 'PASC_count', 'PASC_prop'])
    return df_all

    

@transform_pandas(
    Output(rid="ri.vector.main.execute.e613897a-26d7-4ba6-a44a-39c4e14cd5d1"),
    FINAL=Input(rid="ri.foundry.main.dataset.b055970f-c7df-48d5-9500-ea0849721c29")
)
def booster(FINAL):
    df = FINAL
    booster_columns = [x for x in df.columns if 'vax' in x and 'L' in x]
    df = df.select(*booster_columns)
    df.summary().show()

@transform_pandas(
    Output(rid="ri.foundry.main.dataset.dbee75bb-118f-4abd-86a3-0657910783b1"),
    filter=Input(rid="ri.foundry.main.dataset.e3e8d439-29c5-4975-ab6d-3945c44b47b4")
)
...
@transform_pandas(
    Output(rid="ri.vector.main.execute.7b54d987-b8eb-42e1-951f-a20de34994ad"),
    clean=Input(rid="ri.foundry.main.dataset.dbee75bb-118f-4abd-86a3-0657910783b1")
)
def collapse(clean):
    df = clean
    # C
    ...
    # A
    df = df.withColumn('A_outcome', F.greatest(*A_outcome))
...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.3078f64b-94e4-40e2-a823-bfdcf45a799a"),
    clean=Input(rid="ri.foundry.main.dataset.dbee75bb-118f-4abd-86a3-0657910783b1")
)
...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.e3e8d439-29c5-4975-ab6d-3945c44b47b4"),
    Combined=Input(rid="ri.foundry.main.dataset.050a8c76-0469-4f54-afbd-69c30f355142")
)
def filter(Combined):
    ...
    df = df.filter(df.vax_to_treatment.isNull() | ((df.vax_to_treatment > 1) & (df.vax_to_treatment <= 13)))
    # filter covid+1 month before long covid
    df = df.filter(df.vax_to_treatment.isNull() | ((df.vax_to_treatment+1) < df.vax_to_long) | df.vax_to_long.isNull())
...
@transform_pandas(
    Output(rid="ri.vector.main.execute.b2d690b7-b1e0-4d76-bf1d-97739bfab0a6"),
    clean=Input(rid="ri.foundry.main.dataset.dbee75bb-118f-4abd-86a3-0657910783b1")
)
def summary(clean):
    df = clean
...
@transform_pandas(
    Output(rid="ri.vector.main.execute.a55fdde9-756a-4c08-a49f-847143edb8e6"),
    collapse_diff=Input(rid="ri.foundry.main.dataset.3078f64b-94e4-40e2-a823-bfdcf45a799a")
)
...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.6333a038-838f-4422-96a6-75c6b402da59"),
    collapse_diff=Input(rid="ri.foundry.main.dataset.3078f64b-94e4-40e2-a823-bfdcf45a799a")
)
def unnamed(collapse_diff):
    df = collapse_diff
    df = df.filter(df.vax_to_treatment > 11)
    return df
